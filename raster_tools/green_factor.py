...
def command(gardens_path, aerial_image_path, target_path, min_green, max_green, check_rast, part, pk):
    ...
    if check_sr is None:
        ...
        print('[*] INFO  : Use this command: gdalsrsinfo -o wkt EPSG:28992 > ', gardens_path.replace('.shp', '.prj'))
        return 1

    #check spaces in aerial_image_path
    if ' ' in aerial_image_path:
        print('[!] ERROR : There is a space in the aerial image path: ', aerial_image_path)
        return 1

    # check if raster exists
    if not os.path.isfile(aerial_image_path):
        print('[!] ERROR : The specified file could not be found: ', aerial_image_path)
        return 1

    # get raster info and check rasterformat
    rasterformat = os.path.splitext(aerial_image_path)[1]
    if (rasterformat == '.ecw' or rasterformat == '.tif' or rasterformat == '.vrt'):
        pixelsize, envelope_aerial_image = gdalinfo(aerial_image_path)
    else:
        print('[!] ERROR : No supported raster format selected: ', rasterformat ,' (use .tif, .vrt or .ecw)')
        return 1
    ...
    open(target_path.replace('.shp', '.prj'), 'w').write(osr.GetUserInputAsWKT(str('EPSG:28992')))
    ...
    for count, feature_garden in enumerate(index):
        ...
        skip_large_size = check_envelopes_input(garden_name, envelope_garden, envelope_aerial_image)
        if not skip_large_size == 1:
            ...
            projwin_garden = ' '.join(str(envelope_garden_round[i]) for i in [0,3,1,2])
            ...
            tmp_green_tif = os.path.join(tmp_dirname, garden_name + '_green.tif')

            #prepare rgbt-array for green factor calculation
            rgbt, rasterdata = prepare_aerial_array(tmp_aerial_tif, aerial_image_path, projwin_garden, sr)

            #prepare mask-array for green factor calculation
            z = rgbt[:1, :, :] * 0
            rasterdata['fillvalue']=1
            array = create_filled_rasterarray(z, geometry_garden, tmp_mask_tif, rasterdata, print_rast=0)
            m = array[0].astype('b1')

            # Call function to determine the green factor
            greenfactor, greengarden_percentage, greengarden_pixels = determine_green_factor(rgbt, m, min_green, max_green)
            ...
            if check_rast == 1:
                ...
                polygon_envelope_garden = get_envelope_polygon(envelope_garden_round, rasterdata['sr'])
                outside = polygon_envelope_garden.Difference(geometry_garden)
                create_filled_rasterarray(greenfactor[np.newaxis], outside, tmp_green_tif, rasterdata, print_rast=1)
...
def determine_green_factor(rgbt, m, min_green, max_green):
    ...
    greenfactor = (np.sum([rgbt[0], rgbt[2]], axis = 0) / (np.sum([rgbt[1], rgbt[1]], axis = 0) + 0.0001)).astype('f4')
    green_mask = np.array((greenfactor > (np.zeros(rgbt[0].shape) + min_green))
                          &
...
def create_filled_rasterarray(array, burn_polygon, raster_path, rasterdata, print_rast):
    ...
    with datasets.Dataset(array, **kwargs) as source:
        ...
        burn_value(dataset=source, geometry=burn_polygon, value=rasterdata['fillvalue'])
...
def prepare_aerial_array(tmp_aerial_tif, aerial_image_path, projwin_garden, sr):
...
def check_envelopes_input(garden_name, envelope_garden, envelope_aerial_image):
    ...
    if not (g_x1 > ai_x1 and g_x2 < ai_x2 and g_y1 > ai_y1 and g_y2 < ai_y2):
        print('[*] WARNING : Polygon (%s) is not (completely) within the aerial image envelope' % garden_name)

    #check size of garden
    area = (g_x2 - g_x1) * (g_y2 - g_y1)
    if area > 50000:
        skip_large_size = 1
        print('[*] WARNING : Skip polygon (%s) because size is too large: %s' % (garden_name, area))
    else:
        skip_large_size = 0
...
def ecw_gdal_translate(aerial_image_path, tmp_aerial_tif, projwin_garden):
    ...
    a_srs = '"+proj=sterea +lat_0=52.15616055555555 +lon_0=5.38763888888889 +k=0.9999079 +x_0=155000 +y_0=463000 +ellps=bessel +towgs84=565.4171,50.3319,465.5524,-0.398957388243134,0.343987817378283,-1.87740163998045,4.0725 +units=m +no_ defs"'
    compression = '"COMPRESS=DEFLATE"'
    data_type = 'Byte'
    cmd_str = ['ecw-gdal_translate -projwin %s -a_srs %s -co %s -ot %s %s %s' % (projwin_garden, a_srs, compression, data_type, aerial_image_path, tmp_aerial_tif)]
    #print("\n[!] About to translate an aerial image '%s' to geotiff..." % aerial_image_path)
...
def gdalinfo(aerial_image_path):
    ...
    #print("\n[!] About to get the info of an aerial image '%s' ..." % aerial_image_path)
    #print("[*] The command that will be used for this: %s" % cmd_str)
    try:
        proc = subprocess.Popen(cmd_str, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    grep_pixelsize_line = re.findall("Pixel Size = \([0-9+].[0-9]+", gdalinfolog)[0]
    ...
    envelope_aerial_image[0], envelope_aerial_image[3] = map(float, grep_upper_left_line.split(','))

    grep_upper_left_line = re.findall("Lower Right \((.*)\)", gdalinfolog)[0]
    envelope_aerial_image[1], envelope_aerial_image[2] = map(float, grep_upper_left_line.split(','))
...
def get_parser():
    ...
    # add arguments here
    parser.add_argument('aerial_image_path', metavar='AERIAL_IMAGE', help='input path with spaces to aerial image in ecw-format')
    parser.add_argument('gardens_path', metavar='POLYGON_PATH', help ='path to polygon layer (e.g. gardens) with unique field')
    parser.add_argument('target_path', metavar='TARGET_PATH', help ='output path gardens with green factor')
    parser.add_argument('--min_green', default=0.7, type=float, metavar='MINIMUM_GREEN', help='minimum greenfactor in aerial image (default: 0.7)')
    parser.add_argument('--max_green', default=0.9, type=float, metavar='MAXIMUM_GREEN', help='maximum greenfactor in aerial image (default: 0.9)')
    parser.add_argument('--pk', default='id', metavar='PRIMARY_KEY', help='select another primary key field (default: id)')
    parser.add_argument('--check_rast', action='store_true', help='creates raster with the greenfactor to check the min and max greenfactorsettings')
    parser.add_argument('-p', '--part', help='partial processing source, for example 2/3')
